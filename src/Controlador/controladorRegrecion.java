        ...
        if (e.getSource() == vistaRegrecion.jButton_Calcular) {
            if (modeloX.size() == modeloY.size()) {
                if (modeloX.size() == 0 || modeloY.size() == 0) {
                	...
                	JOptionPane.showMessageDialog(null, "Por favor ingresa el valor de la Variable Independiente");
                	vistaRegrecion.jTextField_VarInd.requestFocus();
                } else {
                	if(esN.esNumero(vistaRegrecion.jTextField_VarInd.getText())) {
                    vistaRegrecion.jTextField_Resultado.setText(String.valueOf(estadisticas.GenRegresion(modeloX, modeloY, Float.parseFloat(vistaRegrecion.jTextField_VarInd.getText()))));
                	}else {
                		JOptionPane.showMessageDialog(null, "La Variable Independiente tiene que ser un numero");
                		vistaRegrecion.jTextField_VarInd.requestFocus();
                		vistaRegrecion.jTextField_VarInd.setText("");
                	}
                }
